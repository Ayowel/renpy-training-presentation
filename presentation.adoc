...
L'ancre d'une image définie avec `anchor` déplace le point de l'image utilisé pour les traitements fait dessus (positionnement avec `pos` ou rotation avec une transform).
--
...
[.notes]
--
Ajouter des boutons dans les préférences pour passer d'un langue à l'autre.
En l'état, le changement de langue ne persistera pas en cas de redémarrage.
